---
title: "Sentiment Analysis (Word to Sentence Level)"
...
alice_sentences <-alice_long %>%
  unnest_tokens(output = ??, input = ??, token = "??") %>%
...
#aggregate scores
alice_summized <- alice_words_sentiments %>%
  group_by(??) %>%
  summarize(??) %>%
  ungroup()
...
alice_summarized %>%
  #add a polarity flag
  mutate(polarity = ifelse(?? > 0, "positive", "negative"))

  #feed it into a basic bar graph
  ggplot(aes(x = ??grouping??, y = ??score??, fill = polarity)) +
