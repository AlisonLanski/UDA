...
```{r cache=TRUE}
df <- find_thread_urls(subreddit = "Pasadena", sort_by = "new", period = "year")
head(df)
```
...
```{r cache=TRUE}
extraction <- get_thread_content(df$url[1:20])
str(extraction)
df_top <- extraction$threads
df_threads <- extraction$comments
```



## Do some cleanup
Take a look at some of the string data columns (titles, text).  You will probably see weird characters or other things that don't look right.  
...
Your turn!
```{r}
# Ok, I don't have particular code here yet; could add some
df_top <- df_top %>%
  #this isn't real code, just an example of what you might see
  mutate(new_title = str_replace_all(title, pattern = "", replacement = ""),
        new_tax = str_replace_all(text, pattern = "", replacement = ""))

#ok, same idea here
df_threads <- df_threads %>%
  mutate(new_comment = comment)
```
...
```{r, eval = FALSE}
df_top$netid <- "alanski"
df_threads$netid <- "alanski"

write_csv(df_top, "C:/Users/alanski/Downloads/reddit_top_posts.csv")
write_csv(df_threads, "C:/Users/alanski/Downloads/reddit_threads.csv")
```
