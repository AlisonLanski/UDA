...
#or in base R, use GREP (Global Regular Expression Print)
grep(x = strings, pattern = "an", value = TRUE)
```
...
```
Challenge: what if the 2-3 "p"s don't have to be sequential?
```{r eval = FALSE}
strings <- c("apple", "banana", "pear", "pineapple", "peach pot pie")
pattern <- ""
str_subset(strings, pattern)
```
