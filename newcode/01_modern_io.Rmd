...
congress_critters <- fromJSON("C:/Users/alanski/Documents/GitHub/UDA/data/critters.json")
...
```{r}
congress_hard <- fromJSON("C:/Users/alanski/Documents/GitHub/UDA/data/critters.json")
```


```{r}
congress_brief <- purrr::map_df(1:100, ~{
  data.frame(id = congress_hard$objects$person$bioguideid[[.x]], 
             link = congress_hard$objects$person$link[[.x]], 
             state = congress_hard$objects$state[[.x]])
})
...
```
To subset before you apply the function, use the "subset of data.table" symbol .SD with .SDcols
This allows us to use lambda notation to apply the function to numerous columns without requiring a lengthy .(mean_x = mean(x), mean_y =  mean(y), mean_z = mean(z))
intead we can say
lapply(.SD, mean),
by = senator_rank,
.SDcols = c("x", "y", "z")
...
write_feather(congress_brief, here::here("data/congress_feather_lanski.feather"))
congress_feather <- arrow::read_feather(here::here("data/congress_feather_lanski.feather"))
str(congress_feather)
str(congress_brief)
```
